...
#define MC__ODEBND_GSL_TMELL_MVUSE
...
( const TVT* TMp )
{
...
#ifdef MC__ODEBND_GSL_TMELL_MVUSE
  // Internal Taylor model environment reset
  if( _MVXPenv && ( _MVXPenv->nord() != _TMenv->nord() 
                 || _MVXPenv->nvar() != _nx+_np ) ){
    delete[] _MVXPx; _MVXPx = 0;
    delete[] _MVXPd; _MVXPd = 0;
    delete[] _MVXPp; _MVXPp = 0;
  ...
  case Options::ELLIPS:
    ...
    if( !_Iddot )   _Iddot   = new T[_nx];
#ifdef MC__ODEBND_GSL_TMELL_MVUSE
    if( !_MVXPenv ) _MVXPenv = new TMT( _nx+_np, _TMenv->nord() );
