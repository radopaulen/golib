...
private:
  ...
  //! @brief interval bounds on Taylor polynomial 
  T *_IPx;
...
public:
  ...
  ODEBND_VAL
    ...
    : ODESTRUCT(IVP()), ODETAYLOR<T,IVP>(), ODETAYLOR<TVT,IVP>(), BASE_ODE()
    {
      ...
      _Ip = _Ix = _Ix_val = _Rxp = _IPx = _Rx = _Rx_TE = _Rinv = _Ninv = 0;
      _TMenv = 0;
      _TMp = _TMx = 0;
  ...
  struct Options: public BASE_GSL::Options
  {
    ...
    Options():
      ...
      USEINV(true), USENBND(false), DISPLAY(0), RESRECORD(false)
      {}
    ...
    bool USEINV;
    //! @brief Whether or not to use the specified natural bounds for bounds contraction (default: true)
    bool USENBND;
    //! @brief Display level
    int DISPLAY;
...
private:
  ...
  bool _invariant
    ( Invariant<TVT>&TVT_I, const TVT*TMx, bool force=false );
  ...
  //! @brief Function to contract enclosures in TMT arithmetic using ODE invariants
  STATUS _TVT_invariant
    ( std::ostream&os );
...
( Invariant<TVT>&TVT_I, const TVT*TMx, bool force )
{
  if( !options.USEINV || !_ni ) return true;

  // Update invariant results if needed
  if( !force && isequal( _t, TVT_I.t ) ) return false;
  ODETAYLOR<TVT,IVP>::invariant( _istg, _t, TMx, _TMp, TVT_I.inv );
  TVT_I.t = _t;
  return true;
}
...
{
  if( !options.USEINV || !_ni ) return;

  // Update invariants
  for( unsigned int ii=0; ii<_ni; ii++ ){
    for( unsigned int jx=0; jx<_nx; jx++ )
      inv[ii] += (dinvdx[ii+jx*_ni]-Op<T>::mid(dinvdx[ii+jx*_ni]))*Rx[jx];
    if( d2invdx2dd )
      inv[ii] += 0.5*d2invdx2dd[ii];
  }
  return;
}

template <typename T, typename IVP> template <typename U> inline double
ODEBND_VAL<T,IVP>::_scale
( U&R, const T&X )
{
...
( TExpansion<U>&U_TE, const T*X )
{
  ...
  std::cout << std::endl << "factorial: " << _factorial(options.TSORDER+1) << std::endl;
#endif
  return std::pow( _smaxnorm( U_TE.f[options.TSORDER+1], X)*_factorial(options.TSORDER+1)
                   + machprec(), -1./(double)options.TSORDER ) * options.HREDUC;
  //return std::pow( _smaxnorm( U_TE.f[options.TSORDER+1], X) + machprec(),
  //                 -1./(double)options.TSORDER ) * options.HREDUC;
}
...
( TExpansion<U>&U_TE, const double&h, const T*X )
{
  ...
  std::cout << "current step: h=" << h << " <=? "
            << std::pow( _smaxnorm( U_TE.f[options.TSORDER+1], X)*_factorial(options.TSORDER+1)
	                 + machprec(), -1./(double)options.TSORDER ) << std::endl;
  std::cout << "U_TE.f[options.TSORDER+1]:";
  for( unsigned int ix=0; ix<_nx; ix++ )
    std::cout << "  " << U_TE.f[options.TSORDER+1][ix];
  std::cout << std::endl;
#endif
  return( h < std::pow( _smaxnorm( U_TE.f[options.TSORDER+1], X) + machprec(),
                        -1./(double)options.TSORDER ) );
}
...
{
  // Stepsize validation
  for( double h = _stepsize_init( T_TE, X ); true; h *= options.HREDUC ){
#ifdef MC__ODEBND_VAL_DEBUG_STEPSIZE
    ...
    for( unsigned int ix=0; ix<_nx; ix++ ){
      _Ix_val[ix] = _predictor( T_TE, ix, h*T(0.,1.) ) + h*options.TOL*T(-1,1);
#ifdef MC__ODEBND_VAL_DEBUG
    ...
    if( _stepsize_valid( U_TE_val, h, X ) ) return h;
  }
}
...
{
  // Stepsize validation
  for( double h = _stepsize_init( TVT_TE, X ); true; h *= options.HREDUC ){
#ifdef MC__ODEBND_VAL_DEBUG
    std::cout << "\nStepsize attempt: h=" << h << std::endl;
#endif
    // Check minimum stepsize criterion
    ...
    // Compute interval enclosure on full step [0,h]
    for( unsigned int ix=0; ix<_nx; ix++ ){
      _Ix_val[ix] = _predictor( TVT_TE, ix, h*T(0.,1.) ).bound() + h*options.TOL*T(-1,1);
#ifdef MC__ODEBND_VAL_DEBUG
      std::cout << "_Ix_val[ix] =" << _Ix_val[ix] << std::endl;
#endif
    }
...
( const double tnext, std::ostream&os )
{
  ...
  for( _h = 0; tnext > _t; _t += _h, ++_nsteps ){
    ...
    case Options::NONE:{
      ...
      _h = _stepsize( _T_TE, _T_TE_val, _Ix );
    ...
    default:{
      ...
      // Stepsize selection and validation
      _h = _stepsize( _TVT_TE, _T_TE_val, _Ix );
      if( _h < options.HMIN ) return FAILURE;
      // Modify stepsize if stage time is exceeded
      if( _t+_h > tnext ) _h = tnext-_t;
      ...
      for( unsigned int ix=0; ix<_nx; ix++ ){
        ...
        // Remainder is 2nd- and higher-order terms *plus* truncation error
        _Rxp[ix] = TMRHSxhi.bound() + _h*options.TOL*T(-1.,1.);
#ifdef MC__ODEBND_VAL_DEBUG
...
ODEBND_VAL<T,IVP>::_TVT_prepare()
{
  ...
  default:
    ...
    if( options.USEINV && _ni ){
      if( !_Ainv.n ) _Ainv.resize(_ni,_nx);
      if( !_Rinv )   _Rinv = new T[_ni];
      if( !_Ninv )   _Ninv = new T[_nx];
    }
    break;
  }
    ...
    default:
     ...
     if( options.ORDMIT <= 2 ){
      ...
      if( options.TSTOP || isequal( _t, _tmax ) ){
        ...
        if( options.ORDMIT <= 1 ){
          ...
          // Stepsize selection and validation
          _hmax = _h = _stepsize( _T_TFE, _T_TE_val, _Ix );
        }
        else{
          _expansion( _T_TFFdE, _Ix, _Rx_TE ); // 2nd-order terms in T arithmetic to reduce CPU time
          for( unsigned int ix=0; ix<_nx; ix++ )
            _IPx[ix] = _TMx[ix].bound(); // Bound on polynomial part
          _expansion( _T_TFE, _IPx ); // 1st-order terms in T arithmetic to reduce CPU time
          _expansion_add( _TVT_TE.f,  _T_TFE.dfdx,  _Rx_TE, _T_TFFdE.d2fdx2dd  );
          // Stepsize selection and validation
          _hmax = _h = _stepsize( _T_TFFdE, _T_TE_val, _Ix );
        }
     ...
     // of the same order as the parameter Taylor model is computed
      ...
      if( options.TSTOP || isequal( _t, _tmax ) ){
        ...
        // Stepsize selection and validation
        _hmax = _h = _stepsize( _TVT_TE, _T_TE_val, _Ix );
        if( _hmax < options.HMIN ) return FAILURE;
        if( _hmax > options.HMAX ) _hmax = _h = options.HMAX;
        // Modify stepsize if stage time is exceeded
        _tmax = _t+_hmax;
      ...
      for( unsigned int ix=0; ix<_nx; ix++ ){
        ...
        _Rx[ix] = TMRHSxhi.bound() + _h*options.TOL*T(-1.,1.);
#ifdef MC__ODEBND_VAL_DEBUG
        std::cout << "_Rx[" << ix << "] =" << _Rx[ix]
                  << "  diam(_Rx[" << ix << "]) =" << Op<T>::diam(_Rx[ix])
                  << "  midp(_Rx[" << ix << "]) =" << Op<T>::mid(_Rx[ix])
     ...
     if( _TVT_invariant( os ) != NORMAL ) return FAILURE;

     // Update Taylor model and Taylor-based bounds
     for( unsigned int ix=0; ix<_nx; ix++ ){
     ...
     // Update interval bounds further with natural bounds
     if( options.USENBND )
       for( unsigned int ix=0; ix<_nx; ix++ ){
         _Ix[ix] = IVP().BND( ix, _Ip, _Ix, _t, _istg );
#ifdef MC__ODEBND_VAL_DEBUG
         std::cout << "_Ix[" << ix << " (aft.) = " << _Ix[ix] << std::endl;
#endif
      }
     //int dum; std::cin >> dum;
...
template <typename T, typename IVP>
inline typename ODEBND_VAL<T,IVP>::STATUS
ODEBND_VAL<T,IVP>::_TVT_invariant
( std::ostream&os )
{
  ...
  // Update Taylor model and Taylor-based bounds
  for( unsigned int ix=0; ix<_nx; ix++ ){
    _TMx[ix].set( T(0.) ); // polynomial part
    _Rx[ix] = T( _Ex.l(ix), _Ex.u(ix) ); // centered remainder
    _Ix[ix] = _TMx[ix].bound() + _Rx[ix];
  }

  // Characterize invariant (basis and type)
  if( options.ORDMIT <= 1 ){
    _invariant( _TVT_I, _TMx );
    _invariant( _T_FI, _Ix ); // 1st-order terms in T arithmetic to reduce CPU time
    _invariant_add( _TVT_I.inv, _T_FI.dinvdx, _Rx );
    for( unsigned int ii=0; ii<_ni; ii++ ){
      for( unsigned int jx=0; jx<_nx; jx++ )
        _Ainv(ii,jx) = Op<T>::mid(_T_FI.dinvdx[ii+jx*_ni]);
      _Rinv[ii] = _TVT_I.inv[ii].remainder();
    }
  }
  else if( options.ORDMIT == 2 ){
    _invariant( _TVT_I, _TMx );
    _invariant( _T_FFdI, _Ix, _Rx ); // 2nd-order terms in T arithmetic to reduce CPU time
    for( unsigned int ix=0; ix<_nx; ix++ )
      _Ix[ix] = _TMx[ix].bound(); // Bound on polynomial part
    _invariant( _T_FI, _Ix ); // 1st-order terms in T arithmetic to reduce CPU time
    _invariant_add( _TVT_I.inv, _T_FI.dinvdx, _Rx, _T_FFdI.d2invdx2dd );
    for( unsigned int ii=0; ii<_ni; ii++ ){
#ifdef MC__ODEBND_VAL_DEBUG_INVARIANT
      std::cout << "_TVT_I.inv[" << ii << "]:" << _TVT_I.inv[ii];
#endif
      for( unsigned int jx=0; jx<_nx; jx++ )
        _Ainv(ii,jx) = Op<T>::mid(_T_FI.dinvdx[ii+jx*_ni]);
      _Rinv[ii] = _TVT_I.inv[ii].remainder();
    ...
    _invariant( _TVT_I, _TMx, _Rx );
    for( unsigned int ii=0; ii<_ni; ii++ ){
      // Extract Taylor model polynomial part in p and set to 0
      TVT invTMx; _TVT_I.inv[ii].get( invTMx.set(_TMenv), true );
      // Extract first-order terms and set to 0
      for( unsigned int jx=0; jx<_nx; jx++ )
  	_Ainv(ii,jx) = _TVT_I.inv[ii].linear(_np+jx,true);
      _Rinv[ii] = _TVT_I.inv[ii].bound();
    }
  }
#ifdef MC__ODEBND_VAL_DEBUG_INVARIANT
    for( unsigned int ii=0; ii<_ni; ii++ )
      std::cout << "_Rinv[" << ii << "] =" << _Rinv[ii] << std::endl;
  std::cout << "_Ainv =\n" << _Ainv << std::endl;
  std::cout << "_Ex (initial) =\n" << _Ex << std::endl;
#endif

  // Contract ellipsoidal remainder <-- NONLINEAR INVARIANTS NOT YET TESTED -->
  for( unsigned int ii=0; ii<_ni; ii++ ){
