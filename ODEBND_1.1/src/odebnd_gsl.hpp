...
( double t, const double* x, double* xdot, void* user_data )
{
  ...
  default:
    ...
    if( !options.ORDMIT ){
      ...
      // Set AD arrays -- Remainder in Taylor model is canceled
      T*Ip = new T[_np];
      for( unsigned int ip=0; ip<_np; ip++ )
        Ip[ip] = _TMp[ip].B();
      FT*dIx = new FT[_nx];
    ...
    // Taylor Expansion of order q in P and mean-value theorem in X with Taylor expansion of lower order
    else if( _TMenv->nord() > _MVXPenv->nord() ){
      ...
      { int dum; std::cin >> dum; }
#endif
      FTVT*dMVXPx = new FTVT[_nx];
      for( unsigned int ix=0; ix<_nx; ix++ ){
        dMVXPx[ix] = _MVXPx[ix];
        dMVXPx[ix].diff(ix,_nx);
      }
      ...
      for( unsigned int ix=0; ix<_nx; ix++ ){
        FTVT dMVXPfi = IVP().RHS( ix, _MVXPp, dMVXPx, t, _istg );
#ifdef MC__ODEBND_GSL_LINTR_DEBUG
        std::cout << "MVXPf[" << ix << "] = " << dMVXPfi.x() << std::endl;
        for( unsigned int jx=0; jx<_nx; jx++ )
          std::cout << "dMVXPf[" << ix << "," << jx << "] = " << dMVXPfi.d(jx) << std::endl;
#endif
        // Extract constant coefficients in X and set to 0
        for( unsigned int jx=0; jx<_nx; jx++ ){
          _A[ix+jx*_nx] = dMVXPfi.d(jx).center().constant();
          dMVXPfi.d(jx) -= _A[ix+jx*_nx];
        }
#ifdef MC__ODEBND_GSL_LINTR_DEBUG
        std::cout << "A[" << ix << ",#] = ";
        for( unsigned int jx=0; jx<_nx; jx++ )
        ...
        _Iddot[ix] = _TMxdot[ix].remainder();
        for( unsigned int jx=0; jx<_nx; jx++ )
          _Iddot[ix] += dMVXPfi.d(jx).bound() * _Ix[jx];
#ifdef MC__ODEBND_GSL_LINTR_DEBUG
        std::cout << "Iddot[" << ix << "] = " << _Iddot[ix]
                  << " : " << Op<T>::mid(_Iddot[ix]) << std::endl;
        std::cout << "kappa[" << ix << "] = "
...
( const TVT* TMp )
{
  ...
  // Internal Taylor model environment reset
  unsigned int MVXPsize = ( options.ORDMIT<_TMenv->nord()? options.ORDMIT: _TMenv->nord() ); 
  if( _MVXPenv && ( _MVXPenv->nord() != MVXPsize || _MVXPenv->nvar() != _nx+_np ) ){
    delete[] _MVXPx; _MVXPx = 0;
    delete[] _MVXPd; _MVXPd = 0;
    delete[] _MVXPp; _MVXPp = 0;
    delete _MVXPenv; _MVXPenv = 0;
  ...
  case Options::ELLIPS:
    ...
    if( !_Qdot )    _Qdot    = new double[_nx*(_nx+1)/2];
    if( !_Iddot )   _Iddot   = new T[_nx];
    if( !_MVXPenv ) _MVXPenv = new TMT( _nx+_np, MVXPsize );
