...
public:
  ...
  ODEBND_VAL
    ...
    : ODESTRUCT(IVP()), ODETAYLOR<T,IVP>(), ODETAYLOR<TVT,IVP>(), BASE_ODE()
    {
      ...
      _Ip = _Ix = _Ix_val = _Rxp = _IPx = _Rx = _Rx_TE = _Rinv = 0;
      _TMenv = 0;
      _TMp = _TMx = 0;
...
//#ifdef MC__ODEBND_VAL_DEBUG_STEPSIZE
  std::cout << "current step: h=" << h << " <=? "
            << std::pow( _smaxnorm( U_TE.f[options.TSORDER+1], X) + machprec(),
                         -1./(double)options.TSORDER ) << std::endl;
  std::cout << "U_TE.f[options.TSORDER+1]:";
  for( unsigned int ix=0; ix<_nx; ix++ )
    std::cout << "  " << U_TE.f[options.TSORDER+1][ix];
  std::cout << std::endl;
...
( std::ostream&os )
{
  ...
  std::cout << "_Ainv =\n" << _Ainv << std::endl;
  std::cout << "_Ex (bef. contraction) =\n" << _Ex << std::endl;
#endif

  // Contract ellipsoidal remainder <-- NONLINEAR INVARIANTS NOT YET IMPLEMENTED -->
  for( unsigned int ii=0; ii<_ni; ii++ ){
    // Affine invariant case
    if( Op<T>::diam(_Rinv[ii]) < machprec() ){
      CPPL::dgematrix Hii( _nx, _nx );
      Hii.identity();
      Hii -= t( _Ainv.row(ii) ) * _Ainv.row(ii) / ( _Ainv.row(ii) % _Ainv.row(ii) );
#ifdef MC__ODEBND_VAL_DEBUG_INVARIANT
      std::cout << "Hii =\n" << Hii << std::endl;
#endif
      _Ex = mtimes( _Ex, Hii );  
      //_Ex = hpintersection( _Ex, std::make_pair( t(_Ainv.row(ii)), 0. ) );
#ifdef MC__ODEBND_VAL_DEBUG_INVARIANT
      std::cout << "> flag: " << _Ex.info() << std::endl;
      std::cout << "_Ex (aft. contraction) =\n" << _Ex << std::endl;
      int dum; std::cin >> dum;
#endif
      if( _Ex.info() ) return FAILURE;
    }
