...
public:  
  ...
  SBB()
    : options(), _np(0), _P_root(0), _P_tmp(0), _node_inc(0), _p_inc(0),
      _f_inc(options.INF)
    {}
  ...
  struct Options
  {
    ...
    enum CRITERION{
      ...
      RELIAB		//!< Reliability branching based on range diameter
    };
    ...
    unsigned int BRANCHING_RELIABILITY_THRESHOLD;
  ...
  std::pair<double,const double*> solve
    ( const TYPE pb, const double*p0=0, std::ostream&os=std::cout );
...
private:  
  ...
  static const unsigned int _IPREC = 6;
  ...
  std::pair<unsigned int, double> _select_branching_variable
    ( SBBNode<T>*pNode, const std::set<unsigned int>&set_branch );
  //! @brief Apply strong branching for branching variable selection
  std::pair<unsigned int, double> _strong_branching
    ( SBBNode<T>*pNode, const std::set<unsigned int>&set_branch );
  ...
  std::pair<const T, const T> _partition_variable_domain
    ( SBBNode<T>*pNode, const unsigned int ip ) const;
...
public:  
  ...
  std::pair<unsigned int,double>& pc_var
    ( const unsigned int ip )
    { assert( ip < 2*_pSBB->_np ); return _pc_var[ip]; }
  //! @brief Retreive variable pseudocosts
  const std::pair<unsigned int,double>* pc_var() const
    { return _pc_var; }
  ...
  SBBNode
    ( SBB<T>*pSBB, const T*P, const double strength,
      ...
      const unsigned int iter, const typename SBB<T>::NODETYPE type,
      const std::pair<unsigned int,T> parent,
      const std::pair<unsigned int,double>* pc_var );
  ...
  bool at_bound
    ( const double val, const unsigned int ip, const double rtol ) const;
...
private:
  ...
  std::pair<unsigned int,double> *_pc_var;
...
SBB<T>::solve
( const TYPE pb, const double*p0, std::ostream&os )
{
  ...
  for( _node_index = 1; !_Nodes.empty() && _tcur-_tstart < options.MAX_CPU_TIME
    ...
    _display_add( (unsigned int)_Nodes.size() );
...
( SBBNode<T>*pNode, const bool relaxed, const bool strongbranching )
{
  _tLBD -= time();
  NODESTAT stat;
  switch( pNode->lower_bound( _f_inc ) ){
...
( SBBNode<T>*pNode, const bool relaxed, const bool strongbranching )
{
  _tUBD -= time();
  NODESTAT stat;
  switch( pNode->upper_bound( _f_inc ) ){
...
( const double fINC, const double*pINC )
{
  ...
  for( unsigned int ip=0; ip<_np; ip++ ) _p_inc[ip] = pINC[ip];//Node->pLB(ip);
...
( SBBNode<T>*pNode, const std::set<unsigned int>&set_branch )
{
  std::pair<unsigned int, double> branchsel( _np, -options.INF );
...
( SBBNode<T>*pNode, const unsigned int ip_branch )
const
{
...
( SBBNode<T>*pNode )
{
  ...
  const unsigned int ip_branch = _select_branching_variable( pNode,
    _branch_set ).first;
...
( SBBNode<T>*pNode, const std::set<unsigned int>&set_branch )
{
  ...
  for( unsigned int ip=0; ip<2*_np; ip++ )
...
SBB<T>::_display_final()
{
  ...
  // Feasible solution found
    ...
    for( unsigned int ip=0, id=0; ip<_np; ip++, id++ ){
      if( id == _LDISP ){
...
( const double inc )
{
  for( unsigned int i=0; i<_pSBB->_np; i++ ) _P0[i] = _P[i];
...
inline bool
SBBNode<T>::at_bound
( const double val, const unsigned int ip, const double rtol )
const
{
  assert( ip < _pSBB->_np );
...
( const typename SBB<T>::TYPE pb )
{
  ...
  unsigned int ipc = _parent.first;
