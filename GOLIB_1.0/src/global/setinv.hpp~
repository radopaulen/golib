...
////////////////////////////////////////////////////////////////////////
//! mc::SetInv is a pure virutal base C++ class implementing a
...
template <typename T, typename USR> struct lt_SetInvNode;
...
////////////////////////////////////////////////////////////////////////
template <typename T, typename USR>
class SetInvNode
////////////////////////////////////////////////////////////////////////
{
  template <typename U, typename NODE, typename LT_NODE> friend class SetInv;
  template <typename U, typename USER> friend struct lt_SetInvNode;
...
public:  
  ...
  unsigned int iter() const
    { return _iter; }
  //! @brief Retreive/set pointer to solution model
  USR*& model()
    { return _model; }
  ...
  typename SetInv< T,SetInvNode<T,USR>,lt_SetInvNode<T,USR> >::NODETYPE& type()
    { return _type; }
  ...
  SetInvNode
    ( SetInv< T,SetInvNode<T,USR>,lt_SetInvNode<T,USR> >*pSetInv, const T*P,
      const unsigned int index=1, const unsigned int iter=0 );

  //! @brief Public constructor (child node)
  SetInvNode
    ( SetInv< T,SetInvNode<T,USR>,lt_SetInvNode<T,USR> >*pSetInv, const T*P,
      const double strength, const unsigned int index,
      const unsigned int depth, const unsigned int iter,
      const typename SetInv< T,SetInvNode<T,USR>,lt_SetInvNode<T,USR> >::NODETYPE type,
      const std::pair<unsigned int,T> parent, USR*model );

  //! @brief Destructor
  virtual ~SetInvNode();

  //! @brief Assess
  typename SetInv< T,SetInvNode<T,USR>,lt_SetInvNode<T,USR> >::STATUS assess();
...
private:
  ...
  SetInv< T,SetInvNode<T,USR>,lt_SetInvNode<T,USR> > *_pSetInv;
  ...
  typename SetInv< T,SetInvNode<T,USR>,lt_SetInvNode<T,USR> >::NODETYPE _type;
  //! @brief Parent node branching variable and range
  std::pair<unsigned int,T> _parent;
  //! @brief Pointer to solution model
  USR* _model;
...
{
  bool operator()
    ( const SetInvNode<T,USR>*Node1, const SetInvNode<T,USR>*Node2 ) const
...
( NODE*pNode )
{
  ...
  NODE*pNodeL = new NODE( this, _P_tmp, pNode->volume(),
    ++_node_cnt, pNode->depth()+1, _node_index, LEFT,
    std::make_pair(ip_branch, pNode->P(ip_branch)), pNode->model() );
  _open_nodes.insert( pNodeL );
  _open_volume += pNodeL->volume();
  
  // Right chold node
  _P_tmp[ip_branch] = partition.second;
  NODE*pNodeR = new NODE( this, _P_tmp, pNode->volume(),
...
template <typename T, typename USR>
inline
SetInvNode<T,USR>::SetInvNode
( SetInv< T,SetInvNode<T,USR>,lt_SetInvNode<T,USR> >*pSetInv, const T*P,
  const unsigned int index, const unsigned int iter ):
_pSetInv( pSetInv ), _depth( 0 ), _index( index ), _iter( iter ),
_type( SetInv< T,SetInvNode<T,USR>,lt_SetInvNode<T,USR> >::ROOT ),
_parent( 0, T(0.) ), _model(0)
{
  _P0 = new T[_pSetInv->np()];
  _P  = new T[_pSetInv->np()];
  for( unsigned int i=0; i<_pSetInv->np(); i++ ) _P[i] = P[i];
  _strength = volume();
}
...
_iter( iter ), _type( type ), _parent( parent ), _model( model )
{
  _P0 = new T[_pSetInv->np()];
  _P  = new T[_pSetInv->np()];
  for( unsigned int i=0; i<_pSetInv->np(); i++ ) _P[i] = P[i];
...
template <typename T, typename USR>
inline typename SetInv< T,SetInvNode<T,USR>,lt_SetInvNode<T,USR> >::STATUS
SetInvNode<T,USR>::assess()
{
  for( unsigned int i=0; i<_pSetInv->np(); i++ ) _P0[i] = _P[i];
