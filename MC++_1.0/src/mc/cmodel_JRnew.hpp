...
template <typename T> inline void interpolation(double* coefmon, const CVar<T>& CV, const unsigned int n) {
  ...
  double mulconst (PI/(2.*double(CV._nord()+1)));
...
template <typename T> inline CVar<T> composition(const double* coefmon, const CVar<T>& CVI) {
  ...
  if (CVI._nord() == 0) CV1 = coefmon[0];
    ...
    CV1 = coefmon[0] + coefmon[1]*CVI;
  else{
    CV1 = coefmon[CVI._nord()];
    CV2 = coefmon[CVI._nord()-1] + CVI*CV1;
    if (CVI._nord() > 2) for( unsigned int i=CVI._nord()-2; i>1; i-=2 ){
      CV1 = coefmon[i] + CVI*CV2 - CV1;
      CV2 = coefmon[i-1] + CVI*CV1 - CV2;
      }

    if (CVI._nord()%2 == 0) {
      CV1 = coefmon[0] + 0.5*CVI*CV2 - CV1;
    }
    else {
      CV1 = coefmon[1] + CVI*CV2 - CV1;
      CV1 = coefmon[0] + 0.5*CVI*CV1 - CV2;
    }
  }
...
( const CVar<T> &CV )
{
  ...
  for (int i(1); i<=CV._nord()+1; i++) {
    fact *= double(i);
    }

  rem = 4.*std::pow(a/2.,double(CV._nord()+1))/fact;
...
( const CVar<T> &CV )
{
  ...
  for (int i(3); i<=CV._nord(); i+=2) {
    coefmon[i-1] = 0.;
    coefmon[i] = coefmon[i]/pow(double(i),2.);
    ub += coefmon[i];
  }

  if (CV._nord()%2==0) coefmon[CV._nord()] = 0.;
