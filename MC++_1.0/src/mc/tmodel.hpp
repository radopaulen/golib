...
public:
  ...
  //! @brief Set multivariate polynomial coefficients and remainder term to those of Taylor variable <tt>TV</tt>, possibly defined in another Taylor model environment with less variables and a different expansion order. Coefficients involving other variables or higher order are initialized to 0 if <tt>reset=true</tt> (default), otherwise they are left unmodified. Higher-order terms in TV are bounded and added to the remainder bound. Use with caution when the Taylor models are scaled (see option mc::TModel::Options::SCALE_VARIABLES) since the polynomial coefficients are then scaled.
  TVar<T>& set
    ( TVar<T>& TV, const bool reset=true )
    { if( !_TM || ( TV._TM && _nvar() < TV._nvar() ) )
        return *this;
      ...
      if( !TV._TM ){ *_bndrem = *TV._bndrem; _update_bndord(); return *this; }
      unsigned int*iexp = new unsigned int[_nvar()];
      for( unsigned int jmon=1; jmon<TV._nmon() && jmon<TV._posord(_nord()+1); jmon++ ){
      ...
      delete[] iexp;
      for( unsigned int iord=_nord()+1; iord<=TV._nord(); iord++ )
        *_bndrem += TV._bndord[iord];
...
/*
  TVar<T>& set
    ...
    { if( !_TM || ( TV._TM && ( _nvar() < TV._nvar() || _nord() < TV._nord() ) ) )
      ...
      if( !TV._TM ){ *_bndrem = *TV._bndrem; _update_bndord(); return *this; }
      unsigned int*iexp = new unsigned int[_nvar()];
      for( unsigned int jmon=1; jmon<TV._nmon(); jmon++ ){
        for( unsigned int ivar=0; ivar<_nvar(); ivar++ )
	  iexp[ivar] = (ivar<TV._nvar()? TV._TM->_expmon[jmon*TV._nvar()+ivar]: 0);
        _coefmon[_TM->_loc_expmon(iexp)] = TV._coefmon[jmon];
      }
...
( const unsigned int nvar, const unsigned int nord )
{
  ...
  _nvar = nvar;
  _nord = nord; 
  _binom = new TM_size[nord?(nvar+nord-1)*(nord+1):nvar];
...
( TVar<T>&TVR, const TVar<T>&TV1, const TVar<T>&TV2 )
{
  ...
  // Second operand not associated to TModel
    ...
    if( !Op<T>::inter(*(TVR._bndrem), R1, B2-TVR.B()) )
      return false;
    if( TVR._TM->options.PROPAGATE_BNDT 
     && !Op<T>::inter( TVR._bndT, TV1._bndT, TV2._bndT ) )
      return false;
    if( TVR._TM->options.CENTER_REMAINDER ) TVR._center_TM();
    return true;
  }
