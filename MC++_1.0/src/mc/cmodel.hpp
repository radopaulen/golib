...
////////////////////////////////////////////////////////////////////////
{
  ...
  template <typename U> friend U funcptr
    ( const U, const unsigned int  );
  template <typename U> friend void interpolation
    ( double*, const CVar<U>&, const unsigned int  );
...
//exact coefficients based on http://www2.maths.ox.ac.uk/chebfun/examples/approx/html/ExactChebCoeffs.shtml (a0 is divided by 2)

  double b(Op<T>::mid(CV.B())), a(Op<T>::u(CV.B())-b), z0sq(-b/a), z0(std::sqrt(z0sq*z0sq-1.)), an, rem;

  CVar<T> CV2( CV._CM, 0. ), CV3( CV._CM, 0. ), CVm2 = (CV-b)*(2./a);

  if(z0sq < 0) z0sq -= z0;
  else z0sq += z0;

  an = 2./(a*z0*std::pow(z0sq,double(CV._nord())));

  //remainder based on sum of geometric series
  rem = std::abs(an)/(std::abs(z0sq)-1.);


  //composition based on http://en.wikipedia.org/wiki/Clenshaw_algorithm#Special_case_for_Chebyshev_series
  if (CV._nord() == 0) CV2 = an/2.;
  else if (CV._nord() == 1)
    ...
    if (CV._nord() > 2) for( unsigned int i=CV._nord()-2; i>1; i-=2 ){
      an *= z0sq;
      CV2 = an + CVm2*CV3 - CV2;
      an *= z0sq;
      CV3 = an + CVm2*CV2 - CV3;
  ...
  if( CV2._CM->options.CENTER_REMAINDER ) CV2._center_CM(); //crashes on order 0 for some reason.
  return CV2;
}

template <typename T> T funcptr(const T x, const unsigned int n) {
  switch( n ){
    case 0: return std::sqrt(x);
    case 1: return std::exp(x);
    case 2: return std::log(x);
    case 3: return std::cos(x);
    case 4: return std::asin(x);
    default: throw typename CModel<T>::Exceptions( CModel<T>::Exceptions::UNDEF );
  }
}

template <typename T> inline void interpolation(double* coefmon, const CVar<T>& CV, const unsigned int n) {

  double b(Op<T>::mid(CV.B())), a(Op<T>::u(CV.B())-b), fx[CV._nord()+1];
  
  //atan(1)*4 = pi
  double mulconst (std::atan(1.)*2./(double(CV._nord()+1)));

  for (unsigned int i(0); i<=CV._nord(); i++) fx[i] = funcptr(a*std::cos(mulconst*(2.*double(i)+1.))+b, n);


  for (unsigned int i(0); i<=CV._nord(); i++) {
    double mulconst2 (std::cos(mulconst*double(i))), mulconst3 (4*std::pow(mulconst2,2)-2), b0(0), b1(0);

    if (CV._nord() == 0) b0 = fx[0];
    else if (CV._nord() == 1)
      b0 =fx[0] + mulconst3*b0 - fx[1] ;
    else {
      b0 = fx[CV._nord()];
      b1 = fx[CV._nord()-1] + mulconst3*b0;

      if (CV._nord() > 2) for( unsigned int j=CV._nord()-2; j>1; j-=2 ){
  ...
  coefmon[i] = 2./double(CV._nord()+1)*mulconst2*b0;
  }

  coefmon[0] *=0.5;
 }

template <typename T> inline CVar<T> composition(const double* coefmon, const CVar<T>& CVI) {

  CVar<T> CV1( CVI._CM, 0. ), CV2( CVI._CM, 0. );

  //composition based on http://en.wikipedia.org/wiki/Clenshaw_algorithm#Special_case_for_Chebyshev_series
  if (CVI._nord() == 0) CV1 = coefmon[0];
  else if (CVI._nord() == 1)
    CV1 = coefmon[0] + 2.*coefmon[1]*CVI;
  else{
    CV1 = coefmon[CVI._nord()];
    CV2 = coefmon[CVI._nord()-1] + 2*CVI*CV1;
    if (CVI._nord() > 2) for( unsigned int i=CVI._nord()-2; i>1; i-=2 ){
      CV1 = coefmon[i] + 2.*CVI*CV2 - CV1;
      CV2 = coefmon[i-1] + 2.*CVI*CV1 - CV2;
      }

    if (CVI._nord()%2 == 0) {
      CV1 = coefmon[0] + CVI*CV2 - CV1;
    }
    else {
      CV1 = coefmon[1] + 2.*CVI*CV2 - CV1;
      CV1 = coefmon[0] + CVI*CV1 - CV2;
    }
  }
...
( const CVar<T>&CV )
{
  ...
  double coefmon[CV._nord()+1];

  interpolation(coefmon, CV, 0);

  double b(Op<T>::mid(CV.B())), a(Op<T>::u(CV.B())-b), rem, ub(0), lb(0);

  for (int i(0); i<=CV._nord(); i++) {
    ub += coefmon[i];
    lb += std::pow(-1.,i)*coefmon[i];
    }

  rem = std::max(std::abs(funcptr(a+b, 0)-ub), std::abs(funcptr(b-a, 0)-lb));

  CVI = (CV-b)/a;

  CV2 = composition(coefmon, CVI);
...
{ 
  ...
  rem = std::max(std::abs(funcptr(a+b, 1)-ub), std::abs(funcptr(b-a, 1)-lb));
...
( const CVar<T>&CV )
{
  ...
  rem = std::max(std::abs(funcptr(a+b, 2)-ub), std::abs(funcptr(b-a, 2)-lb));
...
( const CVar<T> &CV )
{
  ...
  rem = std::max(std::abs(funcptr(a+b, 3)-ub), std::abs(funcptr(b-a, 3)-lb));
...
( const CVar<T> &CV )
{
  ...
  rem = std::max(std::abs(funcptr(a+b, 4)-ub), std::abs(funcptr(b-a, 4)-lb));
